  ...
  return (
    <div>
      ...
      <ol start='6' reversed>
        ...
        />        
        <PublicationLi
          title='Knapp-type obstructions in multilinear fractal Fourier extension'
          coauthors={[
            {
              name: 'I. Oliveira',
        ...
        <PublicationLi
          ...
          year='2026'
          arxiv='https://arxiv.org/abs/2601.05802'
          abstract='The Fourier restriction  problem asks when it is meaningful to restrict the Fourier transform of a function to a given set. Many of the key examples are smooth co-dimension  1 manifolds, although there is increasing interest in fractal sets.  Here we propose a natural intermediary problem where one considers the fractal surface generated by the graph of the additive Brownian sheet in $\mathbb{R}^k$.  We obtain the first non-trivial estimates in this direction, giving both a sufficient condition on the range of $q\in[1,2]$ for the Fourier transform to be $L^{q}(\R^{k+1})\to L^2(G(W))$ bounded and a necessary condition for it to be $L^{q}(\R^{k+1})\to L^p(G(W))$ bounded.  The sufficient condition  is obtained via the Fourier spectrum, which is a family of dimensions that interpolate between the Fourier and Hausdorff dimensions. Our main technical result, which is of interest in its own right, gives a precise formula for the Fourier spectrum of the natural measure on the graph of the additive Brownian sheet, and we apply this result to the Fourier restriction problem. Our restriction estimate  is stronger than the estimate obtained from the well-known Stein--Tomas restriction theorem for all $k\geq3$. We obtain the necessary condition in two different ways, one via the Fourier spectrum and one via an appropriate  Knapp   example.'
          key={4}
        />
        <PublicationLi
      ...
      <ol reversed>
        <PublicationLi
          ...
          article='https://pubs.ams.org/PROC/2026-154-05/S0002-9939-2026-17610-4'
